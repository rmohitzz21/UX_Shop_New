...
if ($name === '') sendResponse('error', 'Freebie name is required.', null, 422);
...
if ($id > 0) {
    ...
    if (!$row) sendResponse('error', 'Freebie not found.', null, 404);
    ...
    if (!$stmt->execute()) sendResponse('error', 'Could not update freebie: ' . $stmt->error, null, 500);
...
if (!$stmt->execute()) sendResponse('error', 'Could not create freebie: ' . $stmt->error, null, 500);
